tetra.view.register('todoView', {
	...
	constr: function(me, app, _) {
		return {
			events: {
					...
					'click': {
						...
						'#todo-list .toggle': function(e, elm) {
							var container = elm.closest('li'),
									target = container.find('label').data('id'),
									value = container.find('label').text(),
									completed;
							container.toggleClass('completed');

							if ( container.hasClass('completed') ) { completed = true; } else { completed = false; }

							app.notify('update todo', { target: target, value: value, completed: completed });
					...
					'blur' : {
						...
						'#todo-list .edit': function(e, elm) {
							var target = elm.siblings('.view').find('label').data('id'),
									value = elm.val(),
									completed = !!elm.closest('li').hasClass('completed');
				...
				controller: {
					'load all todo': function( data ) {
						...
						// Show || hide todo list
						if ( !_('#todo-list').find('li').size() ) { _('#footer').hide(); }
						// Managing 's' on 'item' word
						if ( _('#todo-list').find('li').size() === 1 ) { _('#todo-count span').hide(); } else { _('#todo-count span').show(); }

					},
					'todo updated': function ( data ) {
						_('#todo-list').find("[data-id='"+data.ref+"']").html(data.value);
					}
				}
			},
			...
			methods: {
				init: function() {
					app.notify('get todo', {}); // notify controller view has loaded
				}
			}
		};
	}
